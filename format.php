...
// format_colours - output style tag and javascript
...
echo "</a><ul class='topics colourtopics'>\n"; // format_colour - add 'colourtopics' class
...
if ($thissection->summary or $thissection->sequence or $PAGE->user_is_editing()) {
    ...
    // format_colours - output 'editcolours' icon
    echo '<div class="left side">'.format_colour_edit_icon($course->id, 0).'</div>';
    // format_colours - output 'editcolours' icon
    echo '<div class="right side" >&nbsp;</div>';
    if (!is_null($thissection->name)) { // format_colours - moved this out of 'content' into new 'section-header' div
...
while ($section <= $course->numsections) {
    ...
    if ($showsection) {
        ...
        $left = '<div class="left side">'.$currenttext.$section.format_colour_edit_icon($course->id, $section).'</div>';
        // Note, 'right side' is BEFORE content.
        $right = '<div class="right side">';

        if ($displaysection == $section) {    // Show the zoom boxes
            $right .= '<a href="view.php?id='.$course->id.'&amp;topic=0#section-'.$section.'" title="'.$strshowalltopics.'">'.
                 '<img src="'.$OUTPUT->pix_url('i/all') . '" class="icon" alt="'.$strshowalltopics.'" /></a><br />';
        } else {
            $strshowonlytopic = get_string("showonlytopic", "", $section);
            $right .= '<a href="view.php?id='.$course->id.'&amp;topic='.$section.'" title="'.$strshowonlytopic.'">'.
        ...
        if ($PAGE->user_is_editing() && has_capability('moodle/course:update', get_context_instance(CONTEXT_COURSE, $course->id))) {
            ...
            if ($course->marker == $section) {  // Show the "light globe" on/off
                $right .= '<a href="view.php?id='.$course->id.'&amp;marker=0&amp;sesskey='.sesskey().'#section-'.$section.'" title="'.$strmarkedthistopic.'">'.'<img src="'.$OUTPUT->pix_url('i/marked') . '" alt="'.$strmarkedthistopic.'" /></a><br />';
            } else {
                $right .= '<a href="view.php?id='.$course->id.'&amp;marker='.$section.'&amp;sesskey='.sesskey().'#section-'.$section.'" title="'.$strmarkthistopic.'">'.'<img src="'.$OUTPUT->pix_url('i/marker') . '" alt="'.$strmarkthistopic.'" /></a><br />';
            }

            if ($thissection->visible) {        // Show the hide/show eye
                $right .= '<a href="view.php?id='.$course->id.'&amp;hide='.$section.'&amp;sesskey='.sesskey().'#section-'.$section.'" title="'.$strtopichide.'">'.
                     '<img src="'.$OUTPUT->pix_url('i/hide') . '" class="icon hide" alt="'.$strtopichide.'" /></a><br />';
            } else {
                $right .= '<a href="view.php?id='.$course->id.'&amp;show='.$section.'&amp;sesskey='.sesskey().'#section-'.$section.'" title="'.$strtopicshow.'">'.
                     '<img src="'.$OUTPUT->pix_url('i/show') . '" class="icon hide" alt="'.$strtopicshow.'" /></a><br />';
            }
            if ($section > 1) {                       // Add a arrow to move section up
                $right .= '<a href="view.php?id='.$course->id.'&amp;random='.rand(1,10000).'&amp;section='.$section.'&amp;move=-1&amp;sesskey='.sesskey().'#section-'.($section-1).'" title="'.$strmoveup.'">'.
                     '<img src="'.$OUTPUT->pix_url('t/up') . '" class="icon up" alt="'.$strmoveup.'" /></a><br />';
            }

            if ($section < $course->numsections) {    // Add a arrow to move section down
                $right .= '<a href="view.php?id='.$course->id.'&amp;random='.rand(1,10000).'&amp;section='.$section.'&amp;move=1&amp;sesskey='.sesskey().'#section-'.($section+1).'" title="'.$strmovedown.'">'.
        ...
        if (!has_capability('moodle/course:viewhiddensections', $context) and !$thissection->visible) {   // Hidden for students
            echo '<div class="content">';
            echo $left.$right.get_string('notavailable');
        } else {
            echo '<div class="section-header">'.$left.$right;
            if (!is_null($thissection->name)) { // format_colours - moved this out of 'content' into new div 'section-header'
                echo $OUTPUT->heading(format_string($thissection->name, true, array('context' => $context)), 3, 'sectionname');
            }
            else
            {
               echo $OUTPUT->heading(format_string(get_string('topic').' '.$thissection->section, true, array('context' => $context)), 3, 'sectionname');
            }
            echo '</div>';
            echo '<div class="content">';
            echo '<div class="summary">';
        ...
        echo '<p style="text-align:right"><a href="#topofpage" title="'.get_string('top').'">&#11014;&nbsp;'.get_string('top').'</a></p></div>';
        echo "</li>\n";
    }
